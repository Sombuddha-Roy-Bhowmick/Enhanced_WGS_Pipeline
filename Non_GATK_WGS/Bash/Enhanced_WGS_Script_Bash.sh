...
/data/007_Softwares/Isaac4/Isaac-build/bin/isaac-align -r $Isaac_hg38  -b $out/$input1/fastp_output -f fastq --use-bases-mask y150,y150 -m200 -j $threads --keep-duplicates 1 --mark-duplicates 1  -o $out/$input1/Alignment -t $out/$input1/Alignment/Temp

rm -rf $out/$input1/Alignment/Temp

#Alignment & Markduplication of Tumor Sample

/data/007_Softwares/Isaac4/Isaac-build/bin/isaac-align -r $Isaac_hg38 -b $out/$input2/fastp_output -f fastq --use-bases-mask y150,y150 -m200 -j $threads  --keep-duplicates 1 --mark-duplicates 1  -o $out/$input2/Alignment -t $out/$input2/Alignment/Temp
...
eval "$(conda shell.bash hook)"		# Setting bash for conda environment
conda activate py27		# Activating conda environment

#SV Caller

python /data/007_Softwares/manta-1.6.0.centos6_x86_64/bin/configManta.py --normalBam $out/$input1/BQSR/${input1}.bqsr.bam --tumourBam $out/$input2/BQSR/${input2}.bqsr.bam --referenceFasta $Ref_hg38  --runDir $out/variantCaller/Manta

python $out/variantCaller/Manta/runWorkflow.py  -j $threads

#Somatic Variant Caller

python /data/007_Softwares/strelka-2.9.10.centos6_x86_64/bin/configureStrelkaSomaticWorkflow.py --normalBam $out/$input1/BQSR/${input1}.bqsr.bam --tumourBam $out/$input2/BQSR/${input2}.bqsr.bam --referenceFasta $Ref_hg38 --indelCandidates $out/variantCaller/Manta/results/variants/candidateSmallIndels.vcf.gz  --runDir $out/variantCaller/Strelka2_Somatic 

python $out/variantCaller/Strelka2_Somatic/runWorkflow.py -m local -j $threads

#Germline Variant Caller

python /data/007_Softwares/strelka-2.9.10.centos6_x86_64/bin/configureStrelkaGermlineWorkflow.py --bam $out/$input2/BQSR/${input1}.bqsr.bam --referenceFasta $Ref_hg38  --runDir $out/variantCaller/Strelka2_Germline
